...
def visualize_soft_labels(teacher, dataloader, device):
    ...
    for i in range(4):
        ...
        axes[i, 0].imshow(img)
        axes[i, 0].set_title(f"True Label: {classes[labels[i]]}", fontweight='bold')
        axes[i, 0].axis('off')
...
if __name__ == '__main__':
    ...
    for epoch in range(2):
        ...
        for inputs, labels in trainloader:
            ...
            loss = F.cross_entropy(pruned_model(inputs.to(device)), labels.to(device))
            loss.backward()
            optimizer_prune.step()

    # 將剪枝後的模型進行量化
    pq_model = apply_ptq(pruned_model, testloader)
    metrics_pq = benchmark(pq_model, testloader, torch.device('cpu'), is_quantized=True, model_name="Pruned + PTQ int8")
    ...
    print("\nGrid Search (training 30 epochs per combination to evaluate trend)...")
    for T in T_list:
    ...
    print("\n" + "="*55)
    print(f"Best Combination: T={best_combo[0]}, \u03B1={best_combo[1]} (Acc: {best_acc:.2f}%)")
    print("="*55)