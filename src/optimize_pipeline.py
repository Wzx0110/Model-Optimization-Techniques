...
def benchmark(model, dataloader, device, is_quantized=False, model_name="Model"):
    ...
    val_acc = evaluate_acc(
        model, dataloader, device if not is_quantized else 'cpu')
...
def apply_ptq(model, dataloader):
    ...
    q_model.eval()
    torch.quantization.fuse_modules(q_model, [['conv1', 'bn1', 'relu1'], [
...
def apply_structured_pruning(model, device):
    ...
    p_model.conv2.weight.data = model.conv2.weight.data[keep_idx[1]
                                                        ][:, keep_idx[0], :, :]
    ...
    p_model.conv3.weight.data = model.conv3.weight.data[keep_idx[2]
                                                        ][:, keep_idx[1], :, :]
    ...
    p_model.fc1.weight.data = fc1_w[:,
                                    keep_idx[2], :, :].reshape(512, ch3 * 8 * 8)
...
if __name__ == '__main__':
    ...
    if os.path.exists(baseline_path):
        ...
        print(
            f"Error: {baseline_path} not found. Please run cnn_pytorch.py first.")
        sys.exit(1)

    metrics_baseline = benchmark(
        baseline_model, testloader, device, model_name="Float32 Baseline")

    # PTQ int8
    q_model = apply_ptq(baseline_model, testloader)
    metrics_ptq = benchmark(q_model, testloader, torch.device(
        'cpu'), is_quantized=True, model_name="PTQ int8")
    ...
    for epoch in range(5):
        ...
        print(
            f"Epoch [{epoch+1}/5] - Train Loss: {avg_loss:.4f} - Val Acc: {acc:.2f}%")

    metrics_prune = benchmark(pruned_model, testloader,
                              device, model_name="Pruned Model")

    # Knowledge Distillation
    print("--- Knowledge Distillation ---")
    print("Loading ResNet-50 Teacher (~95% on CIFAR-10)...")
    ...
    for epoch in range(30):
        ...
        for inputs, labels in trainloader:
            ...
            loss = distillation_loss(
                student_logits, teacher_logits, labels, T=4, alpha=0.9)
            loss.backward()
            optimizer_kd.step()
            running_loss += loss.item()
        ...
        print(
            f"Epoch [{epoch+1}/30] - Train Loss: {avg_loss:.4f} - Val Acc: {acc:.2f}%")

    metrics_kd = benchmark(student_model, testloader,
                           device, model_name="Distilled Student")

    # 產出圖表
    print("--- Comparative Charts ---")
    labels_plt = ['Baseline', 'PTQ int8', 'Pruned', 'Distillation']
    accs = [metrics_baseline['acc'], metrics_ptq['acc'],
            metrics_prune['acc'], metrics_kd['acc']]
    sizes = [metrics_baseline['size'], metrics_ptq['size'],
             metrics_prune['size'], metrics_kd['size']]

    fig, ax1 = plt.subplots(figsize=(9, 6))
    ax2 = ax1.twinx()
    ax1.bar(labels_plt, sizes, color='lightgreen', edgecolor='black',
            alpha=0.8, width=0.4, label='Model Size (MB)')
    ax2.plot(labels_plt, accs, color='darkblue', marker='o',
    ...
    print("-" * 60)
    print(
        f"{'1. Float32 Baseline':<22} | {metrics_baseline['acc']:<15.2f} | {metrics_baseline['size']:<15.2f}")
    print(
        f"{'2. PTQ int8':<22} | {metrics_ptq['acc']:<15.2f} | {metrics_ptq['size']:<15.2f}")
    print(
        f"{'3. Pruned Model':<22} | {metrics_prune['acc']:<15.2f} | {metrics_prune['size']:<15.2f}")
    print(
        f"{'4. Distilled Student':<22} | {metrics_kd['acc']:<15.2f} | {metrics_kd['size']:<15.2f}")
    print("="*60 + "\n")
